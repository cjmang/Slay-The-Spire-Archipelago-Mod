    ...
    public static class YesButtonEffect {
        ...
        public static void Postfix(ConfirmPopup __instance, ConfirmPopup.ConfirmType ___type) {
            if (___type == AP_SAVE_RESUME) {
                ...
                {
                    if(ch.chosenClass.name().equalsIgnoreCase(SavePatch.savedChar))
                    {
                        CardCrawlGame.chosenCharacter = ch.chosenClass;
                        APClient.logger.info("Found Character: {}", ch.chosenClass.name());
                        break;
                    }
                }
